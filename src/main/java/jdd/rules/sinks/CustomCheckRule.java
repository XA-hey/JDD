...
import jdd.gadgets.collection.iocd.TransformerUtils;
...
import static jdd.dataflow.DataFlowAnalysisUtils.isDupInvokedMethodInFragment;
import static jdd.gadgets.collection.iocd.TransformerUtils.setSinkInstRecord;
import static jdd.rules.sinks.InvokeCheckRule.isBasicValidTypeForInvokeObj;
        ...
        if (customRulesMap.containsKey("groovy")){
            for (String methodSig: customRulesMap.get("groovy")){
                try {
                    ...
                    TransformerUtils.setSinkInstRecord(sootMethod, true, Arrays.asList(-1),instruments);
                } catch (Exception e) {
                    continue;
                }
        ...
        if (customRulesMap.containsKey("Myface")){
            for (String methodSig: customRulesMap.get("Myface")){
                try {
                    ...
                    TransformerUtils.setSinkInstRecord(sootMethod, true, Arrays.asList(-1,0),instruments);
                }catch (Exception e){
                    continue;
                }
            }
            ...
            for (String methodSig: customRulesMap.get("Bsh")){
                try {
                    ...
                    if (methodSig.contains("bsh.BshMethod")) {
                        ...
                        TransformerUtils.setSinkInstRecord(sootMethod, true, Arrays.asList(0,1), instruments);
                    }
                }catch (Exception e){
                    continue;
                }
